										...
										$("#baocun").click(function(){
											...
											str = str.replace(/}/g, "%7d");
													if(id<=-1){
														...
														window.location.href = "score?type=baocun&score_value="
															+ str;
													}
...
<div class="form-group"> 
		...
		<ul class="pagination">
	<%-- <li   ><a href="score?ye=1&name=${c.name}&sex=${c.sex}&depId=${c.dep.id}&age=${c.age!=-1?c.age:""} "  >首页</a></li> --%>
			<li   ><a href="score?ye=1&name=${c.emp.name}&grade=${c.grade}&depId=${c.dep.id}&proId=${c.pro.id}&value=${c.value!=-1?c.value:""} "  >首页</a></li>
		
			<li  id="pre" ><a href="score?ye=${p.ye-1}&name=${c.emp.name}&grade=${c.grade}&depId=${c.dep.id}&proId=${c.pro.id}&value=${c.value!=-1?c.value:""} "  >上一页</a></li>
			<c:forEach begin="${p.beginYe }" end="${p.endYe }" varStatus="status">
			<li <c:if test="${p.ye==status.index}"> class="active"</c:if>><a 
			href="score?ye=${status.index}&name=${c.emp.name}&grade=${c.grade}&depId=${c.dep.id}&proId=${c.pro.id}&value=${c.value!=-1?c.value:""} ">${status.index}</a></li>
			</c:forEach>
			<li id="next"><a href="score?ye=${p.ye+1 }&name=${c.emp.name}&grade=${c.grade}&depId=${c.dep.id}&proId=${c.pro.id}&value=${c.value!=-1?c.value:""}">下一页</a></li>
	<%-- <li   ><a href="score?ye=${p.maxYe }&name=${c.name}&sex=${c.sex}&depId=${c.dep.id}&age=${c.age!=-1?c.age:""} "  >尾页</a></li> --%>
			<li ><a href="score?ye=${p.maxYe }&name=${c.emp.name}&grade=${c.grade}&depId=${c.dep.id}&proId=${c.pro.id}&value=${c.value!=-1?c.value:""}">尾页</a></li>
